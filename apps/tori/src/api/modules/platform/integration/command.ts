...
export async function createConnection(ctx: ServiceContext, input: CreateConnectionInput) {
  ...
  const existing = await ctx.repositories.integration.findConnectionByOwnerAndProviderAccount({
    ownerUserId: userId,
  ...
  const row = await ctx.repositories.integration.createConnection({
    id: uniqueId(),
    ownerUserId: userId,
    provider: input.provider,
    providerAccountId: input.providerAccountId,
