...
- 身份绑定是否就绪：是否存在未删除的 user binding。
...
- `DELETE /user-bindings/:id`
- `DELETE /channel-bindings/:id`

用户可以移除 user binding 或 channel binding。移除 user binding 后，该外部身份不再参与运行时解析；移除 channel binding 后，该外部频道不再参与投递。
...
- `ownerType = USER` 且 `ownerId = 当前用户`。
- 或 `createdByUserId = 当前用户`。
...
- 至少一个属于当前用户的 active connection。管理员可以使用任意 active connection。
...
当前 Dashboard 表单默认创建 `ownerType = USER` 的用户级订阅。后端会强制用户级订阅的 `ownerId` 等于当前用户；如果创建 `ownerType = CHANNEL` 的订阅，`ownerId` 等于目标 `channelId`，权限仍由 `createdByUserId` 和管理员身份控制。
...
用户可以查看自己拥有或创建的 subscription 详情，以及由它生成的 notification events。管理员可以查看全部未删除 subscription。

### 6.4 启用或禁用订阅

入口：`PATCH /subscription/:id`

订阅拥有者、创建者或管理员可以把 subscription status 改为：
...
如果订阅曾派生后台 task definition，subscription 创建、激活和停用都会触发 platform subscription lifecycle event。平台层根据已注册的 subscription task definition 模板创建或恢复派生 task；停用时会禁用 `metadata.subscriptionId` 指向该 subscription 的 task definition，并取消这些 task definition 下的 pending task runs。业务模块只注册模板和 task handler，不直接消费 subscription lifecycle event。
...
禁用或删除 bot instance 会使其 runtime credential auth 和 notification delivery 停止。按当前资源生命周期设计，bot instance 是可替换基础设施，不能直接摧毁 subscription；受影响的 channel bindings 会进入 `suspended`，等待新 bot 接管。
